...
DATA_ROOT = '/home/ubuntu/data/created_data_custom/' #os.getenv('COCO_IMG_ROOT', '/ssd-playpen/data/mscoco/images/')
MIN_BOXES = 10
MAX_BOXES = 100
...
def doit(detector, raw_images, img_paths):
    with torch.no_grad():
        # Preprocessing
        inputs = []
        for raw_image, img_path in zip(raw_images, img_paths):
            try:
...
def dump_features(writer, detector, pathXid):
    ...
    instances_list, features_list = doit(detector, imgs, img_paths)
...
def load_image_ids(img_root, split_dir):
    ...
    for name in os.listdir(img_root): #[6096:]:
        idx = name.split(".")[0]
        pathXid.append(
                (
...
if __name__ == "__main__":
    ...
    extract_feat('/home/ubuntu/py-bottom-up-attention/demo/data/wordseye/%s_custom_batch.tsv' % args.split, detector, pathXid)
