...
DATA_ROOT = '/home/ubuntu/data/created_data/' #os.getenv('COCO_IMG_ROOT', '/ssd-playpen/data/mscoco/images/')
...
with open(os.path.join(DATA_ROOT,'dataset_wordseye_num_objs_adjusted.json')) as f:
    metadata = json.loads(f.read())
info_list = [(DATA_ROOT+img['filepath']+'/'+img['filename'], img['numobjects']) for img in metadata['images']]
info = dict()
for i,b in info_list:
    info[i] = b
...
def doit(detector, raw_images, img_paths=None, info=None):
    with torch.no_grad():
        # Preprocessing
        inputs = []
        ...
        if (len(probs_list) != len(info)):
            print('FAILED\n\n', img_paths,'\n\n')
            return
        
        for probs, boxes, image_size, num_objects in zip(probs_list, boxes_list, images.image_sizes, info):
...
def dump_features(writer, detector, pathXid):
    ...
    num_objs = [info[img_path] for img_path in img_paths]
    instances_list, features_list = doit(detector, imgs, img_paths, num_objs)
...
def build_model():
    ...
    if args['weight'] == 'mask':
        ...
        cfg.MODEL.WEIGHTS = "https://nlp.cs.unc.edu/models/faster_rcnn_from_caffe_attr_original.pkl"
...
extract_feat('/home/ubuntu/py-bottom-up-attention/demo/data/wordseye/%s_boxesadjusted_batch.tsv' % args['split'], detector, pathXid)
